...
def describe_DescribeNode():
    def it_identifies_test_nodes():
        node = DescribeNode(
            ...
            node_type=NodeType.TEST, nodeid="test::it_works",
        )
    ...
    def it_identifies_describe_nodes():
        node = DescribeNode(
            ...
            node_type=NodeType.DESCRIBE, nodeid="test::describe_Foo",
        )
    ...
    def describe_test_count():
        def it_returns_1_for_test_node():
            node = DescribeNode(
                name="it_works", display_name="it works",
                node_type=NodeType.TEST, nodeid="x",
            )
            assert node.test_count == 1

        def it_counts_children_recursively():
            child1 = DescribeNode(
                name="t1", display_name="t1",
                node_type=NodeType.TEST, nodeid="t1",
            )
            child2 = DescribeNode(
                name="t2", display_name="t2",
                node_type=NodeType.TEST, nodeid="t2",
            )
            inner = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE,
                nodeid="d", children=[child2],
            )
            parent = DescribeNode(
                name="p", display_name="p", node_type=NodeType.DESCRIBE,
                nodeid="p", children=[child1, inner],
            )
            assert parent.test_count == 2

        def it_returns_0_for_empty_describe():
            node = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE, nodeid="d",
            )
    ...
    def describe_passed_count():
        def it_returns_1_for_passed_test():
            node = DescribeNode(
                name="t", display_name="t", node_type=NodeType.TEST, nodeid="t",
        ...
        def it_counts_children():
            c1 = DescribeNode(
                name="t1", display_name="t1", node_type=NodeType.TEST, nodeid="t1",
                result=TestResult(outcome=TestOutcome.PASSED),
            )
            c2 = DescribeNode(
                name="t2", display_name="t2", node_type=NodeType.TEST, nodeid="t2",
                result=TestResult(outcome=TestOutcome.FAILED),
            )
            parent = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE,
                nodeid="d", children=[c1, c2],
            )
    ...
    def describe_failed_count():
        ...
        def it_counts_children():
            ...
            c3 = DescribeNode(
                name="t3", display_name="t3", node_type=NodeType.TEST, nodeid="t3",
                result=TestResult(outcome=TestOutcome.PASSED),
            )
            parent = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE,
                nodeid="d", children=[c1, c2, c3],
            )
    ...
    def describe_find_by_nodeid():
        def it_finds_self():
            node = DescribeNode(
                name="t", display_name="t", node_type=NodeType.TEST, nodeid="target",
            )
            assert node.find_by_nodeid("target") is node

        def it_finds_child():
            child = DescribeNode(
                name="t", display_name="t", node_type=NodeType.TEST, nodeid="child",
            )
            parent = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE,
                nodeid="parent", children=[child],
            )
            assert parent.find_by_nodeid("child") is child

        def it_finds_deeply_nested():
            grandchild = DescribeNode(
                name="t", display_name="t", node_type=NodeType.TEST, nodeid="gc",
            )
            child = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE,
                nodeid="c", children=[grandchild],
            )
            parent = DescribeNode(
                name="d", display_name="d", node_type=NodeType.DESCRIBE,
                nodeid="p", children=[child],
            )
            assert parent.find_by_nodeid("gc") is grandchild

        def it_returns_none_if_not_found():
            node = DescribeNode(
                name="t", display_name="t", node_type=NodeType.TEST, nodeid="x",
            )
